...
const CreateSubCatForm = (props: FormProps) => {
  const {
    register,
    handleSubmit,
  ...
  const mutation = trpc.subCategory.addCategory.useMutation({
    onSuccess({ name }) {
  ...
  const onSubmit: SubmitHandler<FormSchemaType> = async (data) => {
    ...
    await mutation.mutate({ name: data.name, categoryId: data.id });
...
export default CreateSubCatForm;
